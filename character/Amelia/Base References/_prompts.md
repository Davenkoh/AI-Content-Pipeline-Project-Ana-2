# Amelia — Base Reference gen prompts (UK) · 2026-07-26
...
- **`[LOOK]` = British / Northern-European (English).** Registry: `amelia` · `id_prefix: tta` · country **UK**.
- **Her lane (do not drop this — it is what keeps the roster from reading as one girl):** The refined 'English rose' — cool, long-lined, freckled, the most delicate of the five.
...
Save as `Amelia_01_hero-front.png`.

```text
Editorial agency test-shot, full-resolution studio portrait of a stunning, conventionally attractive
22-year-old British / Northern-European (English) woman — clearly an adult woman in her early twenties with mature adult facial
proportions, never reads younger, never a teenager. Cool porcelain-rosy complexion with a natural flush high on the cheeks and a faint scattering of tiny freckles across the bridge of her nose. A longer oval face with a delicately defined narrow jaw, soft high cheekbones, a slim straight nose, blue-grey eyes with slightly down-turned outer corners, and a delicate sharply defined cupid's-bow upper lip over a medium lower lip.
Healthy radiant glow, a slim well-proportioned figure with a natural full C-cup bust, lifted and accentuated with a push-up bra worn under the tank (tasteful, body-conscious fit), with realistic fabric tension and natural folds across the ribbed knit. Long sleek dark chocolate-brown hair, center part with soft face-framing
...
Skin realism: natural skin with fine pores and real texture, subtle asymmetry, two small identity-defining beauty marks — one just above the tail of her left eyebrow and one small one on her right collarbone, realistic
...
Save as `Amelia_02_face-closeup.png`. Solo, full-res.

```text
Same woman from the attached reference image — identical face and identity, same cool porcelain-rosy skin tone,
...
Save as `Amelia_03_half-front.png`. Gives the scene-gen a body/proportion anchor.
...
Save as `Amelia_04_vary-angles.png`. Batched as one 2×2 sheet to save gens. `prep_cover_refs.py` picks
...
- **Realism guardrail** — the dewy "glass-skin" makeup is the fastest path back to the waxy-AI look; the
  ...
  **editorial** (studio). Her *scene* photos (`chars/amelia_cover.png` / `_ending.png`) are a different
...
python3 engine/drive/drive_sync.py --mirror "character/Amelia" --dest "Amelia"
```
