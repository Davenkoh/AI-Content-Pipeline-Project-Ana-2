# Violet — Base Reference gen prompts (France) · 2026-07-26
...
- **`[LOOK]` = French / Western-European.** Registry: `violet` · `id_prefix: ttv` · country **France**.
- **Her lane (do not drop this — it is what keeps the roster from reading as one girl):** The cool fine-boned Parisian — narrow, angular and understated; the least 'glossy' of the five. Do NOT write her as 'gamine': that word pulled candidate 1 under the age gate (read ~18-19). Narrow + fine-boned must be carried by ADULT structure — cheek hollows, a defined jaw, no baby fat.
...
Save as `Violet_01_hero-front.png`.

```text
Editorial agency test-shot, full-resolution studio portrait of a stunning, conventionally attractive
22-year-old French / Western-European woman — clearly an adult woman in her early twenties with mature adult facial
proportions, never reads younger, never a teenager. Light neutral-olive skin with a matte-leaning finish. A narrow, fine-boned Parisian face with a fully mature adult bone structure and no soft baby fat: subtly angular cheekbones with a visible hollow beneath them, a cleanly defined slim jawline, a small softly pointed chin, a slim straight nose, slightly hooded grey-green eyes set a touch narrow with a calm self-possessed adult gaze, and a small mouth with a full natural pout. She carries herself like a woman in her twenties, not a teenager.
...
Skin realism: natural skin with fine pores and real texture, subtle asymmetry, two small identity-defining beauty marks — one on her left jawline and one small one below her right eye, realistic
...
Save as `Violet_02_face-closeup.png`. Solo, full-res.

```text
Same woman from the attached reference image — identical face and identity, same light neutral-olive skin tone,
...
Save as `Violet_03_half-front.png`. Gives the scene-gen a body/proportion anchor.
...
Save as `Violet_04_vary-angles.png`. Batched as one 2×2 sheet to save gens. `prep_cover_refs.py` picks
...
- **Realism guardrail** — the dewy "glass-skin" makeup is the fastest path back to the waxy-AI look; the
  ...
  **editorial** (studio). Her *scene* photos (`chars/violet_cover.png` / `_ending.png`) are a different
...
python3 engine/drive/drive_sync.py --mirror "character/Violet" --dest "Violet"
```
