...
> sequences it. `CONTRACT.md` + `knowledge/frameworks/content_frameworks.md` are the output spec. Read
> `WORKFLOW.md` first; do not restate its commands here.

## Touches
- **Knowledge (load at §4 copy):** `CONTRACT.md`, `knowledge/frameworks/content_frameworks.md`,
  `knowledge/copy_bank/<country>/<OPT>.md`, `knowledge/tuning/06_performance.md`,
  `knowledge/tuning/02_copywriting.md`, `knowledge/voice/humanizer.md`, the Sheet **Holicay Brand** tab.
  **§5 sourcing:** `knowledge/tuning/03_sourcing.md`, `knowledge/frameworks/photo_sourcing.md`, `CONTRACT.md`
...
- **Copy dial** — `same` (verbatim reuse of a pack) | `similar` (paraphrase — same places, reworded) |
  ...
  across accounts. This drives the §4 copy_bank freshness rule.
...
- **§4** copy — load the CONTRACT schema + this framework's `content_frameworks.md` block + the
  `copy_bank/<country>/<OPT>.md` history (honor the copy dial) + `tuning/06` + `02` + `voice/humanizer` +
  the live **Holicay Brand** tab → write `outputs/<key>/<ID - Title>/copy.json` (**pre-broken lines**) +
  `caption.txt` (title verbatim + one-para-per-slide essay + hashtag formula; **C1: keyword CTA is the
...
- **§5** photos — plan slugs (`fixtures/places.json` shape) → source via `brightdata.py places`
  (prioritized) / `gimg` / `ig` (+ `app_icons.py` + `route_map_shot.js` for C2) → **VISION-curate against
...
- **§8 VISION GATE:** `Read` `_work/render/_contact.png` against **CONTRACT gates 1–8**; fix + re-render on any miss.
...
- **§10** deliver — `drive_sync --post` (prints the Drive link) → `sheets.py post-upsert` (all slot fields +
  Drive link + caption) → **append the `iteration N` entry** to `copy_bank/<country>/<OPT>.md`.
...
to Drive, with a logged Sheet row and an advanced `copy_bank` history. The human reviews on the Sheet +
