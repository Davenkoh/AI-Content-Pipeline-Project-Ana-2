...
4. **Instagram via Apify (`engine/source/brightdata.py ig`) — fallback / deep-cut UGC** when Places and SerpAPI run thin: real posted phone shots found by hashtag or location. (The 1.0-era scrape.do/Bing proxy fallback was dropped in 2.0.)
...
- **Source a real background** (Pexels/Unsplash first — watermark-free; reject tiled-watermark stock). This is the same source-it-real discipline as §5.2–5.3, now for the *scene the character stands in*.
